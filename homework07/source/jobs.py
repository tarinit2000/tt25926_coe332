...
q = HotQueue("queue", host='10.100.138.84', port=6379, db=1)
rd = StrictRedis(host='10.100.138.84', port=6379, db=0)
...
def set_workerIP(jid, ip):
    """saves the worker's IP address as new key in redis db"""
    rd.hset(_generate_job_key(jid), 'worker', ip)
